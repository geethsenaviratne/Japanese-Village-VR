...
public class ReturnPoint : MonoBehaviour
{
    ...
    public AudioClip bladeReturnSound;     // NEW
    ...
    {
        Debug.Log("=== TURNING ON GROUND LANTERNS (SAME AS BOX001) ===");
        List<GameObject> groundLanterns = new List<GameObject>();

        Color brightYellow = new Color(1f, 0.95f, 0.7f); // Bright yellow/white
        ...
        if (groundLanternsParent != null)
        {
            ...
            foreach (Light light in lights)
            {
                ...
                light.range = 15f; // Increased range
                light.type = LightType.Point;
            }
            ...
            foreach (Renderer renderer in renderers)
            {
                if (renderer.material != null)
                {
                    ...
                    // Enable emission (same as Box001)
                    renderer.material.EnableKeyword("_EMISSION");
                    renderer.material.SetColor("_EmissionColor", brightYellow * 4f);

                    // Set base color
                    Color baseColor = renderer.material.GetColor("_BaseColor");
                    if (baseColor != null)
                    {
                        renderer.material.SetColor("_BaseColor", brightYellow);
                    }
                }
            }

            Debug.Log("Turned on " + lights.Length + " ground lanterns (same style as Box001)");
            return;
        }
        ...
        Debug.Log("Activated " + groundLanterns.Count + " ground lanterns with " + lightsActivated + " lights (same style as Box001)!");
    }
    ...
    void TurnOnHouseWindows()
    {
        ...
        Debug.Log("=== TURNING ON HOUSE1 WINDOWS (SAME AS BOX001) ===");

        // Turn on all lights in House1
        Light[] lights = house1.GetComponentsInChildren<Light>(true);
        foreach (Light light in lights)
        {
            light.enabled = true;
            light.color = windowGlowColor; // Same as Box001
            if (light.intensity < 2f)
            {
                light.intensity = 3f; // Same as Box001
            }
        }

        // Get all renderers in House1
        ...
        foreach (Renderer renderer in renderers)
        {
            if (renderer.material != null)
            {
                ...
                // Check for window, glass, or element2 (element2 is the window object in House1)
        ...
        Debug.Log("Activated " + windowCount + " window materials in House1 (same style as Box001)!");
    }
    ...
    void ReturnBlade()
    {
        ...
        if (audioSource != null && bladeReturnSound != null)
        {
            audioSource.PlayOneShot(bladeReturnSound);  // NEW: play audio
        }
